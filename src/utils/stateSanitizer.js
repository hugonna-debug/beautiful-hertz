export const DEFAULT_TALENT_NODES = [
...
export const DEFAULT_PRESTIGE_UPGRADES = {
...
export function migrateItem(item) {
  ...
  let rawSubstats = Array.isArray(item.substats) ? item.substats : [];
  const cleanSubstats = rawSubstats
    .map(sub => {
      if (!sub || typeof sub !== 'object') return null;
      const type = validTypes.includes(sub.type) ? sub.type : 'percent_atk';
      ...
      return { type, value: val, locked, rarity: subRarity };
    })
    .filter(s => s !== null);
...
export function sanitizeAndMigrateState(parsedInput, defaultStateTemplate) {
  ...
  migratedTalents = DEFAULT_TALENT_NODES.map(defNode => {
    const existing = parsedTalents.find(t => t && t.id === defNode.id);
  ...
  const lootBackpack = rawBackpack.map(migrateItem).filter(item => item !== null);
  ...
  // LPC Character config migration
  const mergedLpc = {
    ...DEFAULT_LPC_CHARACTER,
    ...(parsed.lpcCharacter || {})
  };
  if (mergedLpc.torso === 'shirt_white') {
    mergedLpc.torso = 'none';
  }
  ...
  const sanitizedState = {
    ...
    combatLogs: Array.isArray(parsed.combatLogs) ? parsed.combatLogs.filter(l => typeof l === 'string') : ['Portal matrix initialized. Auto-clashing active.'],
    dpsMeter,
    difficultyTier,
    hero: null,
